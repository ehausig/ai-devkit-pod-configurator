...
select_build_command() {
    ...
    echo "Available container build commands:"
    echo ""
    ...
    if [ ${#commands[@]} -eq 1 ]; then
        echo "Only one working command found, using it automatically."
        echo "${commands[0]}"
        return 0
    fi
...
main() {
    ...
    if [[ -f "$CONFIG_FILE" ]]; then
        ...
        echo ""
        cat "$CONFIG_FILE" | grep -E "build_command:|runtime:|runtime_import:" | sed 's/^/  /'
        echo ""
...
# Generated: $(date)
# To reconfigure: ./configure-container-runtime-v2.sh
