...
select_languages() {
    ...
    while true; do
        ...
        if [[ $page_end -gt $total_items ]]; then
        ...
        echo ""
        echo -e "${YELLOW}Navigation:${NC} ↑/↓ or j/k    ${YELLOW}Select:${NC} SPACE    ${YELLOW}Done:${NC} ENTER    ${YELLOW}Quit:${NC} q"
        if [[ $total_pages -gt 1 ]]; then
            echo -e "${YELLOW}Pages:${NC} ← → or h/l    ${BLUE}Page $((current_page + 1)) of $total_pages${NC}"
        fi
        echo -e "${BLUE}────────────────────────────────────────────────────────────────${NC}"
        echo ""
        ...
        echo -e "${GREEN}Selected: $selected_count items${NC}"
        ...
        if [[ $key == $'\e' ]]; then
            ...
            # vim-style left (previous page)
            if ((current_page > 0)); then
                ((current_page--))
                current=$((current_page * page_size))
            fi
        elif [[ "$key" == "l" ]] || [[ "$key" == "L" ]]; then
            # vim-style right (next page)
            if ((current_page < total_pages - 1)); then
                ((current_page++))
                current=$((current_page * page_size))
            fi
    ...
    # Confirmation prompt
    echo -e "${YELLOW}Ready to build with the above configuration?${NC}"
