...
test_zombie_process_handling() {
    ...
    # Create a zombie process simulator
    cat > "/tmp/create-zombie-$$.sh" << 'EOF'
#!/bin/bash
# Parent process that creates a zombie
sleep 0.1 &
# Exit without waiting - creates zombie
exit 0
EOF
    chmod +x "/tmp/create-zombie-$$.sh"
    
    # Create mock actor that becomes zombie
    cat > "/tmp/test-actors-$$/architect-actor" << 'EOF'
#!/bin/bash
echo "Mock architect starting"
es-event-emit "PERSONA_ACTIVATED" "PERSONA:ARCHITECT|PID:$$"
# Simulate becoming a zombie by having parent exit
(/tmp/create-zombie-$$.sh)
exit 0
EOF
    chmod +x "/tmp/test-actors-$$/architect-actor"
    
    # First activation creates zombie
    ...
    # Should activate again despite zombie
...
test_handoff_processing() {
    ...
    # Create work and handoff BEFORE monitor starts
