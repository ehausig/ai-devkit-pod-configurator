...
# Source this file in other scripts: source /usr/local/bin/claude-code-common.sh
...
get_current_persona() {
    local persona=$(journal-query.sh current-persona 2>/dev/null)
...
# Log with timestamp (wrapper around journal-log.sh)
log_event() {
    local tag="$1"
    local message="$2"
    journal-log.sh "$tag" "$message"
}
...
get_pending_count() {
    ...
    journal-query.sh pending-work "$persona" 2>/dev/null | wc -l
}
...
get_next_persona() {
    ...
    case "$current" in
        ...
        DEVELOPER) 
            ...
            if journal-query.sh recent-context DEVELOPER | grep -q "changes requested"; then
                echo "QA"
            else
                echo "QA"
            fi
        ...
        QA)
            ...
            local failed=$(journal-query.sh recent-context QA | grep -c "QA:FAILED" || echo "0")
            if [ $failed -eq 0 ]; then
                echo "REVIEWER"
            else
                echo "DEVELOPER"
        ...
        REVIEWER)
            # Check if approved
            local issues=$(journal-query.sh recent-context REVIEWER | grep -c "REVIEWER:ISSUE" || echo "0")
            if [ $issues -eq 0 ]; then
                echo "MERGER"
            else
                echo "DEVELOPER"
...
check_safety_limits() {
    local persona="${1:-$(get_current_persona)}"
    local result=$(journal-query.sh safety-check "$persona")
    local exit_code=$?
