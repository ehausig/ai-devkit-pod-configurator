...
PENDING_CHECK=$(es-journal-query.sh handoff-ready DEVELOPER)
HANDOFF_READY=$?

if [ $HANDOFF_READY -ne 0 ]; then
    echo -e "${RED}$PENDING_CHECK${NC}"
    es-journal-log.sh "HANDOFF:BLOCKED" "DEVELOPER has incomplete work items"
    exit 1
fi
...
if [ "$TEST_PASSED" = false ]; then
    ...
    if [ -f "test-output.log" ]; then
        ...
        es-journal-log.sh "DEVELOPER:TEST_FAILED" "Tests failing: $FAILURES"
    fi
fi
...
if [ "$READY" = false ]; then
    ...
    es-journal-log.sh "HANDOFF:BLOCKED" "Missing requirements: $MISSING"
    exit 1
fi

# Validation passed
es-journal-log.sh "HANDOFF:VALIDATED" "All DEVELOPER requirements met"
...
COMPLETED_WORK=$(es-journal-query.sh recent-context DEVELOPER | grep "WORK:COMPLETED" | tail -10)
WORK_COUNT=$(echo "$COMPLETED_WORK" | wc -l)

# Create QA work items based on what was implemented
es-journal-log.sh "WORK:PENDING" "QA: Pull branch $CURRENT_BRANCH and set up test environment"
es-journal-log.sh "WORK:PENDING" "QA: Run unit test suite and verify coverage"
es-journal-log.sh "WORK:PENDING" "QA: Start backend services for integration testing"
es-journal-log.sh "WORK:PENDING" "QA: Run integration tests against REAL services (no mocks)"
es-journal-log.sh "WORK:PENDING" "QA: Perform user simulation testing"
es-journal-log.sh "WORK:PENDING" "QA: Test error handling and edge cases"
es-journal-log.sh "WORK:PENDING" "QA: Check performance and resource usage"
es-journal-log.sh "WORK:PENDING" "QA: Document any bugs or issues found"
es-journal-log.sh "WORK:PENDING" "QA: Create test report with pass/fail decision"

# Count QA work items
QA_WORK_ITEMS=$(es-journal-query.sh pending-work QA | wc -l)
...
## Work Items for QA
$(es-journal-query.sh pending-work QA | sed 's/.*WORK:PENDING\] QA: /- /')
...
## Known Issues
$(es-journal-query.sh errors DEVELOPER 5 | sed 's/.*\[\(.*\)\] /- [\1] /' || echo "None reported")

## QA Focus Areas
Based on the implementation, pay special attention to:
$(es-journal-query.sh decisions ARCHITECT 3 | grep -E "(critical|important|security)" | sed 's/.*DECISION\] /- /' || echo "- Standard testing procedures apply")
EOF
