...
PENDING_CHECK=$(es-journal-query.sh handoff-ready MERGER)
HANDOFF_READY=$?

if [ $HANDOFF_READY -ne 0 ]; then
    echo -e "${RED}$PENDING_CHECK${NC}"
    es-journal-log.sh "HANDOFF:BLOCKED" "MERGER has incomplete tasks"
    exit 1
fi
...
MERGED=$(es-journal-query.sh recent-context MERGER | grep -c "MERGER:MERGED" || echo "0")
RELEASES=$(es-journal-query.sh recent-context MERGER | grep -c "MERGER:RELEASE" || echo "0")
ISSUES=$(es-journal-query.sh recent-context MERGER | grep -c "MERGER:ISSUE" || echo "0")
...
# Get merge details
MERGE_DETAILS=$(es-journal-query.sh recent-context MERGER | grep "MERGER:MERGED")
COMPLETED_WORK=$(es-journal-query.sh recent-context MERGER | grep "WORK:COMPLETED")
...
## Releases Created
$(es-journal-query.sh recent-context MERGER | grep "MERGER:RELEASE" | sed 's/.*\[MERGER:RELEASE\] /- /' || echo "No releases created")
...
# Log completion
es-journal-log.sh "HANDOFF:COMPLETED" "Development cycle complete. $MERGED merges, $RELEASES releases"
es-journal-log.sh "MERGER:CONTEXT" "Merge cycle complete. Version $CURRENT_VERSION"
...
if [ "$PENDING_PRS" -gt 0 ]; then
    ...
    es-journal-log.sh "WORK:PENDING" "DEVELOPER: Review open PRs and continue development"
    es-journal-log.sh "WORK:PENDING" "DEVELOPER: Address any post-merge issues"
...
PERSONA_FLOW=$(es-journal-query.sh recent-context | grep "PERSONA:INIT" | tail -10 | sed 's/.*\[\(.*\):INIT\].*/  → \1/')
echo "$PERSONA_FLOW"
echo ""
...
for persona in ARCHITECT DEVELOPER QA REVIEWER MERGER; do
    count=$(es-journal-query.sh recent-context $persona | grep -c "WORK:COMPLETED" || echo "0")
