...
RECENT_TESTS=$(es-journal-query.sh recent-context QA | grep -E "(PASSED|FAILED)" | tail -5)
...
if [ $PENDING_COUNT -gt 0 ]; then
    ...
    echo ""
    echo "3. Log result:"
    echo "   es-journal-log.sh 'QA:PASSED' 'Test description' OR"
    echo "   es-journal-log.sh 'QA:FAILED' 'Test description - reason'"
    echo ""
    ...
    echo ""
    echo "2. Run persona-qa-handoff.sh if testing is complete"
fi
...
echo "• View work: es-journal-query.sh pending-work QA"
echo "• Track item: es-work-tracker.sh track '<test-pattern>'"
echo "• Check all: es-journal-query.sh work-summary QA"
echo "• Get context: es-context-window.sh QA"
echo ""
