...
generate_work_items() {
    ...
    case "$to" in
        ...
        DEVELOPER)
            ...
            local issue_count=$(grep -c "\[REVIEWER:ISSUE\]" "$JOURNAL_FILE" 2>/dev/null | tr -d '\n' | grep -o '[0-9]*' | head -1)
            ...
            if [ "$issue_count" -gt 0 ]; then
                ...
                for ((i=1; i<=$ISSUES_FOUND; i++)); do
                    echo "Address review issue #$i: Fix identified problem"
                done
            fi
...
if [ "$TEST_MODE" = "1" ]; then
    ...
    export -f test_init_reviewer
    
    echo "REVIEWER Actor ready for testing" >&2
fi
