...
if [ "$FAILED" -eq 0 ] && [ "$ISSUES" -eq 0 ]; then
    ...
    journal-log.sh "HANDOFF:REQUEST" "QA requesting handoff to REVIEWER - all tests passed"
else
    NEXT_PERSONA="DEVELOPER"
    echo -e "${YELLOW}Issues found. Returning to DEVELOPER for fixes.${NC}"
    journal-log.sh "HANDOFF:REQUEST" "QA requesting handoff to DEVELOPER - $FAILED failures, $ISSUES issues"
fi
...
if [ $HANDOFF_READY -ne 0 ]; then
    ...
    journal-log.sh "HANDOFF:BLOCKED" "QA has incomplete test items"
    exit 1
fi
...
if [ "$READY" = false ]; then
    ...
    journal-log.sh "HANDOFF:BLOCKED" "Testing requirements not met: $MISSING"
    exit 1
fi

# Validation passed
journal-log.sh "HANDOFF:VALIDATED" "All QA requirements met"
...
if [ "$NEXT_PERSONA" = "REVIEWER" ]; then
    ...
    journal-log.sh "WORK:PENDING" "REVIEWER: Clone PR branch to review directory"
    journal-log.sh "WORK:PENDING" "REVIEWER: Run automated code quality checks (lint, security)"
    journal-log.sh "WORK:PENDING" "REVIEWER: Review code against architectural decisions"
    journal-log.sh "WORK:PENDING" "REVIEWER: Check test quality and coverage"
    journal-log.sh "WORK:PENDING" "REVIEWER: Verify error handling and edge cases"
    journal-log.sh "WORK:PENDING" "REVIEWER: Review documentation completeness"
    journal-log.sh "WORK:PENDING" "REVIEWER: Provide feedback or approve PR"
    ...
    if [ $FAILED -gt 0 ]; then
        ...
        journal-query.sh recent-context QA | grep "QA:FAILED" | while read -r failure; do
            ...
            journal-log.sh "WORK:PENDING" "DEVELOPER: Fix failing test - $failure_desc"
        done
    fi
    ...
    if [ $ISSUES -gt 0 ]; then
        ...
        journal-query.sh recent-context QA | grep "QA:ISSUE" | while read -r issue; do
            ...
            journal-log.sh "WORK:PENDING" "DEVELOPER: Address issue - $issue_desc"
        done
    fi
    
    # Always add re-test item
    journal-log.sh "WORK:PENDING" "DEVELOPER: Run all tests locally to verify fixes"
    journal-log.sh "WORK:PENDING" "DEVELOPER: Update PR with fixes"
...
journal-log.sh "QA:CONTEXT" "Testing complete. $PASSED passed, $FAILED failed, $ISSUES issues found"
